...
void LoginOperation::processOpertion()
{
	...
	myInfo.csNickName = util::stringToCString(userInfo.user_nickname());
	myInfo.avatarUrl = userInfo.user_headlink();
	myInfo.dId = util::uint32ToString(userInfo.friend_groupid());
	myInfo.department = myInfo.dId;
	myInfo.email = "";
	myInfo.gender = userInfo.user_gender();
	myInfo.user_domain = "";
	myInfo.telephone = userInfo.user_phone();
	myInfo.status = 0;
    myInfo.signature = "";
	...
	LOG__(APP, _T("login succeed! Name = %s Nickname = %s sId = %s status = %d")
		...
		, util::stringToCString(userInfo.user_nickname())
