...
void UserListModule_Impl::_allUserlistResponse(IN string& pbBody)
{
	...
	if (imAllUserRsp.latest_update_time() != time)//比较latest_update_time
	{
		...
		for (int n = 0; n < imAllUserRsp.user_list_size(); ++n)
		{
			...
			userInfoEntity.avatarUrl = userInfo.user_headlink();
			userInfoEntity.csName = util::stringToCString(userInfo.user_nickname());
			userInfoEntity.csNickName = util::stringToCString(userInfo.friend_remark());
			userInfoEntity.department = util::uint32ToString(userInfo.friend_groupid());
			userInfoEntity.email = "";
			userInfoEntity.gender = userInfo.user_gender();
			userInfoEntity.user_domain = "";
			userInfoEntity.telephone = userInfo.user_phone();
			userInfoEntity.status = 0;
            userInfoEntity.signature = "";
			userInfoEntity.onlineState = userInfo.user_status();
...
void UserListModule_Impl::_departmentResponse(IN std::string& pbBody)
{
	...
	if (imDepartmentRsp.latest_update_time() != time)//比较latest_update_time
	{
		...
		for (int n = 0; n < imDepartmentRsp.dept_list_size(); ++n)
		{
			...
			departmentEntity.dId = util::uint32ToString(departInfo.group_id());
			departmentEntity.priority = departInfo.group_sort();
			departmentEntity.parentDepartId = "";
			departmentEntity.name = util::stringToCString(departInfo.group_name());
			departmentEntity.status = 0;
			{
				CAutoLock lock(&m_lock);
...
BOOL UserListModule_Impl::startup()
{
	...
	for (const module::UserInfoEntity& userInfo : userlist)
	{
		...
		{
			if (m_mapUsers.find(userInfo.sId)==m_mapUsers.end())
