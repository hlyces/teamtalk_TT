...
BOOL FileTransferUIThread::acceptFileTransfer(const std::string& taskId)
{
	...
	if (pFileSocket)
	{
			...
			[=]()
		{
			...
			{
				int mode = fileEntity.nClientMode == IM::BaseDefine::ClientFileRole::CLIENT_OFFLINE_DOWNLOAD ? IM::BaseDefine::FileType::FILE_TYPE_OFFLINE : IM::BaseDefine::FileType::FILE_TYPE_ONLINE;
				...
				imFilePullDataReq.set_trans_mode(static_cast<IM::BaseDefine::FileType>(mode));
