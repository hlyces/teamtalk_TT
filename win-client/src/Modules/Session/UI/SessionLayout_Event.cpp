	...
	else if (msg.sType == DUI_MSGTYPE_CLICK)
	{
		...
		else if (msg.pSender == m_pBtnSendImage)
		{
			...
			while (nPos != NULL)
			{
				...
				SIZE size = { m_pInputRichEdit->GetWidth(), m_pInputRichEdit->GetHeight() };
				m_pInputRichEdit->InsertImage(sPathName.GetBuffer(), size, FALSE);
...
void SessionLayout::OnFinishScreenCapture(__in LPCTSTR lpFilePath)
{
    ...
    CxImage img;
    img.Load(lpFilePath, CXIMAGE_SUPPORT_PNG);
