...
{
	int nPos = picPath.Find(_T("://"));
	if (-1 != nPos)
	{
		picPath = picPath.Mid(nPos + 3);

		nPos = picPath.Find(_T("/"));
		if (-1 != nPos)
			picPath = picPath.Mid(nPos + 1);
	}
	CString strAddedMSG;
...
int UIIMEdit::onDropFiles(HDROP hDrop)
{
	int i;
	int cch;
	...
	for (i = 0; i < iFiles; i++)
	{
		...
		LOG__(DEBG, _T("%s %d"), lpszFile, iFiles);
		//operation about lpszFile	
		CString strFileName = lpszFile;

		CString strExt = strFileName.Mid(strFileName.ReverseFind(_T('.')) + 1).MakeLower();
		if (strExt == _T("png")
			|| strExt == _T("jpeg")
			|| strExt == _T("jpg")
			|| strExt == _T("gif")
		...
		{//发图片
			...
			this->InsertImage(strFileName.GetBuffer(), size, FALSE);
		...
		{//传文件
			...
			module::getFileTransferModule()->sendFile(strFileName, m_sId, userInfo.isOnlne());
		}
	}
	DragFinish(hDrop);
