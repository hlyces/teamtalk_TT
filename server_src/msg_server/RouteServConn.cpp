...
void update_master_route_serv_conn()
{
	...
	if (g_master_rs_conn)
	{
		...
		pdu.SetCommandId(DFFX_CID_OTHER_ROLE_SET);
...
void CRouteServConn::OnConfirm()
{
	...
	for (list<user_stat_t>::iterator it = online_user_list.begin(); it != online_user_list.end(); it++)
	{
		...
		IM::BaseDefine::UserStat* server_user_stat = msg.add_user_stat_list();
	...
	pdu.SetPBMsg(&msg);
	pdu.SetServiceId(DFFX_SID_OTHER);
	pdu.SetCommandId(DFFX_CID_OTHER_ONLINE_USER_INFO);
	SendPdu(&pdu);
}
...
void CRouteServConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId())
	{
		case DFFX_CID_OTHER_HEARTBEAT:
			break;
		case DFFX_CID_OTHER_SERVER_KICK_USER:
		...
		case DFFX_CID_SWITCH_P2P_CMD:
			_HandleP2PMsg(pPdu );
			break;
		case DFFX_CID_OTHER_LOGIN_STATUS_NOTIFY:
...
void CRouteServConn::_HandleUsersStatusResponse(CImPdu* pPdu)
{
	...
	else if (attach_data.GetType() == ATTACH_TYPE_HANDLE_AND_PDU_FOR_FILE)
	{
		...
		else
		{
			...
			pdu2.SetCommandId(DFFX_CID_FILE_RESPONSE);
...
void CRouteServConn::_HandlePCLoginStatusNotify(CImPdu *pPdu)
{
	...
	if (pUser)
	{
		...
		CImPdu pdu;
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(IM::BaseDefine::DFFX_SID_BUDDY_LIST);
		pdu.SetCommandId(IM::BaseDefine::DFFX_CID_BUDDY_LIST_PC_LOGIN_STATUS_NOTIFY);
		pUser->BroadcastPduToMobile(&pdu);
	}
}

void CRouteServConn::_HandleClientAddFriendRequest(CImPdu* pPdu)
{
	IM::Buddy::IMAddFriendReq msg;
	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
	
	log("_HandleClientAddFriendRequest, from_user_id=%u to_user_id=%u ", msg.from_user_id(), msg.to_user_id());		
