...
void CFileServConn::OnConfirm()
{
	...
	pdu.SetCommandId(DFFX_CID_OTHER_FILE_SERVER_IP_REQ);
	SendPdu(&pdu);
}
...
void CFileServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId())
	{
		case DFFX_CID_OTHER_HEARTBEAT:
			break;
		case DFFX_CID_OTHER_FILE_TRANSFER_RSP:
			_HandleFileMsgTransRsp(pPdu);
			break;
		case DFFX_CID_OTHER_FILE_SERVER_IP_RSP:
			_HandleFileServerIPRsp(pPdu);
			break;
		default:
			log("unknown cmd id=%u ", pPdu->GetCommandId());
			break;
	}
}
...
void CFileServConn::_HandleFileMsgTransRsp(CImPdu* pPdu)
{
	...
	log("HandleFileMsgTransRsp, result: %u, from_user_id: %u, to_user_id: %u, file_name: %s, "\
        "file_size: %d task_id: %s, trans_mode: %u. ", result, from_id, to_id,
	    file_name.c_str(), file_size, task_id.c_str(), trans_mode);
	...
	if (result == 0 && IM::BaseDefine::FILE_TYPE_ONLINE==trans_mode)
	{
		...
		pdu2.SetCommandId(DFFX_CID_FILE_NOTIFY);
