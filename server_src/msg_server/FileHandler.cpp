...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu)
{
	...
	log("HandleClientFileRequest, %u->%u, fileName: %s, fileSize= %d, trans_mode: %u.", from_id, to_id, file_name.c_str(), file_size, trans_mode);
    ...
    if(file_size > 200*1024*1024 || file_size<=0)
    {
		...
		pdu.SetCommandId(DFFX_CID_FILE_RESPONSE);
	...
	if (pFileConn)
	{
		...
		pdu.SetCommandId(DFFX_CID_OTHER_FILE_TRANSFER_REQ);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		...
		if (IM::BaseDefine::FILE_TYPE_OFFLINE == trans_mode)
		{
			...
			//log("HandleClientFileRequest cmd_test = %u", DFFX_CID_OTHER_FILE_TRANSFER_REQ);
		...
		else //IM::BaseDefine::FILE_TYPE_ONLINE
		{
			...
			if (pUser ) //已有对应的账号登录状态  //&& pUser->GetPCLoginStatus())//已有对应的账号pc登录状态
			{
				pFileConn->SendPdu(&pdu);
			}
			else//无对应用户的登录状态,向route_server查询状态
			{
...
void CFileHandler::HandleClientFileHasOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
	...
	else
	{
		...
		pdu.SetPBMsg(&msg);
		pdu.SetServiceId(DFFX_SID_FILE);
		pdu.SetCommandId(DFFX_CID_FILE_HAS_OFFLINE_RES);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pMsgConn->SendPdu(&pdu);
	}
}
...
void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
	...
	// '/' '\\'
	int nRealNamePos = file_name.rfind('/');
	if(nRealNamePos==string::npos)
	{
		nRealNamePos = file_name.rfind('\\');
	}
	if(nRealNamePos!=string::npos)
	{
		file_name = file_name.substr(nRealNamePos+1);
	}
	...
	//if (pFileConn)
	{
		...
		{
			IM::BaseDefine::IpAddr ip_addr_tmp = msg.ip_addr_list(i);//*it;
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_FILE);
		pdu.SetCommandId(DFFX_CID_FILE_NOTIFY);
...
void CFileHandler::HandleClientFileDelOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
	...
	uint32_t to_id = pMsgConn->GetUserId();
...
void CFileHandler::HandleFileNotify(CImPdu* pPdu)
{
	...
	log("HandleFileNotify, from_id: %u, to_id: %u, file_name: %s, file_size: %d, task_id: %s, trans_mode: %u,\
        offline_ready: %u. ", from_user_id, to_user_id, file_name.c_str(), file_size, task_id.c_str(),
