...
void CGroupChat::HandleClientGroupNormalRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	else
	{
		...
		pdu.SetCommandId(DFFX_CID_GROUP_NORMAL_LIST_RESPONSE);
...
void CGroupChat::HandleClientGroupInfoRequest(CImPdu *pPdu, CMsgConn* pFromConn)
{
	...
	else
	{
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_GROUP);
		pdu.SetCommandId(DFFX_CID_GROUP_INFO_RESPONSE);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pFromConn->SendPdu(&pdu);
	}
}
...
void CGroupChat::HandleGroupInfoResponse(CImPdu* pPdu)
{
	...
	if (pduAttachData.GetPduLength() > 0 && group_cnt > 0)
	{
		...
		pdu.SetCommandId(DFFX_CID_MSG_DATA);
		...
		pdu2.SetCommandId(DFFX_CID_OTHER_GET_SHIELD_REQ);
		CDBServConn* pDbConn = get_db_serv_conn();
		if (pDbConn)
		{
...
void CGroupChat::HandleGroupMessage(CImPdu* pPdu)
{
	...
	pdu.SetPBMsg(&msg3);
	pdu.SetServiceId(DFFX_SID_GROUP);
	pdu.SetCommandId(DFFX_CID_GROUP_INFO_REQUEST);
	CDBServConn* pDbConn = get_db_serv_conn();
	if(pDbConn)
	{
		pDbConn->SendPdu(&pdu);
...
void CGroupChat::HandleClientGroupCreateRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	else
	{
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_GROUP);
		pdu.SetCommandId(DFFX_CID_GROUP_CREATE_RESPONSE);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pFromConn->SendPdu(&pdu);
	}
}
...
void CGroupChat::HandleClientGroupChangeMemberRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	else
	{
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_GROUP);
		pdu.SetCommandId(DFFX_CID_GROUP_CHANGE_MEMBER_RESPONSE);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pFromConn->SendPdu(&pdu);
	}
}
...
void CGroupChat::HandleGroupChangeMemberResponse(CImPdu* pPdu)
{
	...
	if (!result)
	{
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_GROUP);
		pdu.SetCommandId(DFFX_CID_GROUP_CHANGE_MEMBER_NOTIFY);
		CRouteServConn* pRouteConn = get_route_serv_conn();
		if (pRouteConn)
		{
...
void CGroupChat::HandleClientGroupShieldGroupRequest(CImPdu *pPdu, CMsgConn *pFromConn)
{
	...
	else
	{
		...
		pdu.SetPBMsg(&msg2);
		pdu.SetServiceId(DFFX_SID_GROUP);
		pdu.SetCommandId(DFFX_CID_GROUP_SHIELD_GROUP_RESPONSE);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pFromConn->SendPdu(&pdu);
	}
}
...
void CGroupChat::HandleGroupGetShieldByGroupResponse(CImPdu *pPdu)
{
	...
	pdu.SetCommandId(DFFX_CID_OTHER_GET_DEVICE_TOKEN_REQ);
	CDBServConn* pDbConn = get_db_serv_conn();
	if (pDbConn)
	{
		pDbConn->SendPdu(&pdu);
