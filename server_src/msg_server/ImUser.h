...
public:
	...
	void SetUserUid(uint32_t user_uid)
	{
		m_user_uid = user_uid;
	}
	uint32_t GetUserUid()
	...
	uint32_t GetPCLoginStatus()
	{
		return m_pc_login_status;
	}
	...
	void DelMsgConn(uint32_t handle)
	{
		m_conn_map.erase(handle);
	}
	...
	map<uint32_t, CMsgConn*>& GetMsgConnMap()
	{
		return m_conn_map;
	}
