...
void init_msg_conn(uint32_t push_id_min, uint32_t push_id_max)
{
	g_push_id_max = push_id_max;
	g_push_id_min = push_id_min;
...
void CMsgConn::SendUserStatusUpdate(uint32_t user_status)
{
	...
	if (user_status == ::IM::BaseDefine::USER_STATUS_ONLINE)
	{
		...
		pdu.SetCommandId(DFFX_CID_OTHER_USER_CNT_UPDATE);
...
{
	log("Close client, handle=%d, user_id=%u m_login_name=%s", m_handle, GetUserId(), m_login_name.c_str());
...
void CMsgConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId())
	{
		...
		case DFFX_CID_MSG_LIST_REQUEST:
			...
			break;
		case DFFX_CID_MSG_GET_BY_MSG_ID_REQ:
		...
		case DFFX_CID_MSG_READ_ACK:
			...
			break;
		case DFFX_CID_MSG_GET_LATEST_MSG_ID_REQ:
		...
		case DFFX_CID_BUDDY_LIST_GETADDFRIEND_REQ:
			_HandleClientGetAddFriendRequest(pPdu);
			break;
		case DFFX_CID_BUDDY_LIST_FINDUSERINFO_REQ:
			_HandleClientFindUserInfoRequest(pPdu);
			break;
...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
	...
	if (result)
	{
		...
		pdu.SetCommandId(DFFX_CID_LOGIN_RES_USERLOGIN);
	...
	log("HandleLoginReq, user_name=%s, status=%u, client_type=%u, client_version=%s, ",
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_OTHER);
	pdu.SetCommandId(DFFX_CID_OTHER_VALIDATE_REQ);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	pDbConn->SendPdu(&pdu);
}
...
void CMsgConn::_HandleLoginOutRequest(CImPdu *pPdu)
{
	...
	if (pDBConn)
	{
		...
		pdu.SetPBMsg(&msg);
		pdu.SetServiceId(DFFX_SID_LOGIN);
		pdu.SetCommandId(DFFX_CID_LOGIN_REQ_DEVICETOKEN);
		pdu.SetSeqNum(pPdu->GetSeqNum());
		pDBConn->SendPdu(&pdu);
	}
	...
	pdu2.SetCommandId(DFFX_CID_LOGIN_RES_LOGINOUT);
...
void CMsgConn::_HandleKickPCClient(CImPdu *pPdu)
{
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_LOGIN);
	pdu.SetCommandId(DFFX_CID_LOGIN_RES_KICKPCCLIENT);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	SendPdu(&pdu);
}
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu)
{
	...
	if (msg.msg_data().length() > MAX_MSG_LEN
		&& msg.msg_type()!=IM::BaseDefine::MSG_TYPE_SINGLE_AUDIO
		&& msg.msg_type()!=IM::BaseDefine::MSG_TYPE_GROUP_AUDIO
		)
	{
		log("too long text message len=%d, uid=%u ", msg.msg_data().length(), GetUserId());
		return;
	}

	uint32_t from_user_id = msg.from_user_id();
	if (m_msg_cnt_per_sec >= MAX_MSG_CNT_PER_SECOND 
		&& (from_user_id < g_push_id_min || from_user_id > g_push_id_max))
	{
		log("!!!too much msg cnt in one second, uid=%u ", GetUserId());
		return;
	}

	if ( CHECK_MSG_TYPE_PUSH(msg.msg_type()) 
		&& (from_user_id < g_push_id_min || from_user_id > g_push_id_max))
	{
		log("!!!no privilege,not pushid, uid=%u ", GetUserId());
		return;
	}
	...
	if(CHECK_MSG_TYPE_PUSH( msg_type))
	{
		...
		if (pToImUser)
		{
			pToImUser->BroadcastClientMsgData(pPdu, msg_id, NULL, GetUserId());
		}
...
void CMsgConn::_HandleClientTimeRequest(CImPdu* pPdu)
{
	...
	pdu.SetPBMsg(&msg);
	pdu.SetServiceId(DFFX_SID_MSG);
	pdu.SetCommandId(DFFX_CID_MSG_TIME_RESPONSE);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	SendPdu(&pdu);
}
...
void CMsgConn::_HandleClientMsgReadAck(CImPdu* pPdu)
{
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_MSG);
	pdu.SetCommandId(DFFX_CID_MSG_READ_NOTIFY);
	CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(GetUserId());
	if (pUser)
	{
		pUser->BroadcastPdu(&pdu, this);
...
void CMsgConn::_HandleClientRemoveSessionRequest(CImPdu* pPdu)
{
	...
	if (session_type == IM::BaseDefine::SESSION_TYPE_SINGLE)
	{
		...
		pdu.SetCommandId(DFFX_CID_BUDDY_LIST_REMOVE_SESSION_NOTIFY);
		CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(GetUserId());
		if (pImUser)
		{
...
void CMsgConn::_HandleClientDeviceToken(CImPdu *pPdu)
{
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_LOGIN);
	pdu.SetCommandId(DFFX_CID_LOGIN_RES_DEVICETOKEN);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	SendPdu(&pdu);
...
void CMsgConn::_HandleClientReverseAddFriendRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleClientFriendNotifyRes(CImPdu* pPdu)
{
	IM::Buddy::IMFriendNotifyRes msg;
	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

	log("from_user_id=%u to_user_id=%u ", msg.from_user_id(), msg.to_user_id());
...
void CMsgConn::_HandleClientChgFriendmarkRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleClientAddFriendGroupRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleClientMoveFrinedGroupRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleClientGetAddFriendRequest(CImPdu* pPdu)
{
...
void CMsgConn::_HandleClientFindUserInfoRequest(CImPdu* pPdu)
{
