...
namespace DB_PROXY {
    ...
    void getRecentSession(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_BUDDY_LIST);
            pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_BUDDY_LIST_RECENT_CONTACT_SESSION_RESPONSE);
    ...
    void deleteRecentSession(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(IM::BaseDefine::SessionType_IsValid(nType))
            {
				...
				{
                    bRet = CSessionModel::getInstance()->removeSession(nUserId, nPeerId, nType);
                ...
                pPduResp->SetSeqNum(pPdu->GetSeqNum());
                pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_BUDDY_LIST);
                pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_BUDDY_LIST_REMOVE_SESSION_RES);
                CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
            }
            else
            {
