...
namespace DB_PROXY {
    ...
    void createGroup(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(IM::BaseDefine::GroupType_IsValid(nGroupType))
            {
                ...
                pPduRes->SetServiceId(IM::BaseDefine::DFFX_SID_GROUP);
                pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_GROUP_CREATE_RESPONSE);
    ...
    void getNormalGroupList(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            pPduRes->SetSeqNum(pPdu->GetSeqNum());
            pPduRes->SetServiceId(IM::BaseDefine::DFFX_SID_GROUP);
            pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_GROUP_NORMAL_LIST_RESPONSE);
            CProxyConn::AddResponsePdu(conn_uuid, pPduRes);
        }
        else
        {
    ...
    //            *pGroupInfo = *it;
            ...
            pPduRes->SetSeqNum(pPdu->GetSeqNum());
            pPduRes->SetServiceId(IM::BaseDefine::DFFX_SID_GROUP);
            pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_GROUP_INFO_RESPONSE);
            CProxyConn::AddResponsePdu(conn_uuid, pPduRes);
        }
        else
        {
    ...
    void modifyMember(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (IM::BaseDefine::GroupModifyType_IsValid(nType) &&
                ...
                pPduRes->SetSeqNum(pPdu->GetSeqNum());
                pPduRes->SetServiceId(IM::BaseDefine::DFFX_SID_GROUP);
                pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_GROUP_CHANGE_MEMBER_RESPONSE);
                CProxyConn::AddResponsePdu(conn_uuid, pPduRes);
            }
            else
            {
    ...
    void setGroupPush(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if(CGroupModel::getInstance()->isValidateGroupId(nGroupId))
            {
                ...
                pPduRes->SetSeqNum(pPdu->GetSeqNum());
                pPduRes->SetServiceId(IM::BaseDefine::DFFX_SID_GROUP);
                pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_GROUP_SHIELD_GROUP_RESPONSE);
                CProxyConn::AddResponsePdu(conn_uuid, pPduRes);
            }
            else
            {
    ...
    void getGroupPush(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        //            *pStatus = *it;
                ...
                pPduRes->SetCommandId(IM::BaseDefine::DFFX_CID_OTHER_GET_SHIELD_RSP);
                CProxyConn::AddResponsePdu(conn_uuid, pPduRes);
            }
            else
            {
