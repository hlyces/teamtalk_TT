                               ...
                               uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
	...
	if (nRelateId != INVALID_VALUE)
    {
        ...
        if (pDBConn)
        {
        	nMsgCnt = nMsgCnt > 100? 100: nMsgCnt;
    ...
    {
        log("no relation between %u and %u", nUserId, nPeerId);
    }
}
...
bool CMessageModel::sendMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, 
	IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, string& strMsgContent, bool isBlack)
{
    bool bRet =false;
...
bool CMessageModel::sendAudioMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, 
	IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, const char* pMsgContent, uint32_t nMsgLen, bool isBlack)
{
	if (nMsgLen <= 4) {
		return false;
	}
...
void CMessageModel::incMsgCount(uint32_t nFromId, uint32_t nToId)
{
	...
	if (pCacheConn) {
		pCacheConn->hincrBy("unread:toId_" + int2string(nToId), int2string(nFromId), 1);
...
void CMessageModel::getUnreadMsgCount(uint32_t nUserId, uint32_t &nTotalCnt, list<IM::BaseDefine::UnreadInfo>& lsUnreadCount)
{
    ...
    if (pCacheConn)
    {
        ...
        string strKey = "unread:toId_" + int2string(nUserId);
...
uint32_t CMessageModel::getMsgId(uint32_t nRelateId)
{
    ...
    if(pCacheConn)
    {
        string strKey = "msg_id:relateId_" + int2string(nRelateId);
...
void CMessageModel::getLastMsg(uint32_t nFromId, uint32_t nToId, uint32_t& nMsgId, string& strMsgData, IM::BaseDefine::MsgType& nMsgType, uint32_t nStatus)
{
    ...
    {
        log("no relation between %u and %u", nFromId, nToId);
    }
}
...
bool CMessageModel::cleanMsgList(uint32_t nUserId, uint32_t nPeerId)
{
	bool bRet = false;
    ...
    {
        string strTableName = "IMMessage_" + int2string(nRelateId % 8);
		string strSql = "update " + strTableName + " set status=1 where status=0 and relateId = " + int2string(nRelateId);
