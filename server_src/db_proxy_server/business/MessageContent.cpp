...
//                    log("userId=%u, peerId=%u, msgId=%u", nUserId, nPeerId, it->msg_id());
				...
				pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
				pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_LIST_RESPONSE);
			...
			if (IM::BaseDefine::MsgType_IsValid(nMsgType))
			{
				if(nMsgLen != 0)
				{
					        ...
					        || nMsgType== IM::BaseDefine::MSG_TYPE_FILE_TRANSFER)
					{
						if (nFromId != nToId)
						{
							...
							if(nMsgType== IM::BaseDefine::MSG_TYPE_SINGLE_TEXT
								...
								|| nMsgType== IM::BaseDefine::MSG_TYPE_FILE_TRANSFER)
							{
								isBlack = CUserModel::getInstance()->isInBlackGroup( nToId, nFromId);
							...
							if( nRelateId != INVALID_VALUE)
							{
								...
								if(nMsgId != INVALID_VALUE)
								{
									   ...
									   || nMsgType== IM::BaseDefine::MSG_TYPE_ORDER_RESULT)
									{
										...
										if(pCacheConn)
										{
											string strTokenKey = "account_uid_"+int2string(nToId);
											string replayPC = pCacheConn->hget(strTokenKey, "token_pc");
											string replayMobile = pCacheConn->hget(strTokenKey, "token_mobile");

											if(replayPC.empty() && replayMobile.empty())
											{
					...
					else if(nMsgType == IM::BaseDefine::MSG_TYPE_SINGLE_AUDIO)
					{
						...
						if(nFromId != nToId)
						{
							...
							if(nSessionId != INVALID_VALUE && nRelateId != INVALID_VALUE)
							{
								...
								if(nMsgId != INVALID_VALUE)
								{
									pMsgModel->sendAudioMessage(nRelateId, nFromId, nToId, nMsgType, nCreateTime, nMsgId, msg.msg_data().c_str(), nMsgLen, isBlack);
									CSessionModel::getInstance()->updateSession(nSessionId, nNow);
									if(!isBlack)
									{
										CSessionModel::getInstance()->updateSession(nPeerSessionId, nNow);
					   ...
					   && nMsgType != IM::BaseDefine::MSG_TYPE_ORDER_RESULT)
					{
						...
						pPduResp->SetSeqNum(pPdu->GetSeqNum());
						pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
						pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_DATA);
						CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
					}
					else
					{
	...
	/*
		...
		if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
		{
			...
			if (IM::BaseDefine::SessionType_IsValid(nType))
			{
				...
				pPduResp->SetSeqNum(pPdu->GetSeqNum());
				pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
				pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_GET_BY_MSG_ID_RES);
				CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
			}
			else
			{
	...
	/*
		...
		if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
		{
			...
			if (IM::BaseDefine::SessionType_IsValid(nType))
			{
				...
				pPduResp->SetSeqNum(pPdu->GetSeqNum());
				pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
				pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_GET_LATEST_MSG_ID_RSP);
				CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
...
void cleanMsgList(CImPdu* pPdu, uint32_t conn_uuid)
	{
		...
		if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
		{
			...
			{
				CImPdu* pPduResp = new CImPdu;
				IM::Message::IMCleanMsgListRsp msgResp;
				msgResp.set_user_id(nUserId);
				msgResp.set_session_type(nType);
				msgResp.set_session_id(nPeerId);

				int result = 0;
				if(!CMessageModel::getInstance()->cleanMsgList(nUserId, nPeerId))
					result = 1;
				
				msgResp.set_result_code(result);
				
				log("userId=%u, peerId=%u, sessionType=%u, result=%u", nUserId, nPeerId, nType, result);
				
				msgResp.set_attach_data(msg.attach_data());
				pPduResp->SetPBMsg(&msgResp);
				pPduResp->SetSeqNum(pPdu->GetSeqNum());
				pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
				pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_CLEAN_MSGLIST_RES);
				CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
