...
namespace DB_PROXY {
    ...
    void getUnreadMsgCounter(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            log("userId=%d, unreadSessionCnt=%u, totalUnreadMsgCount=%u", nUserId, msgResp.unreadinfo_list_size(), nTotalCnt);
            msgResp.set_attach_data(msg.attach_data());
            pPduResp->SetPBMsg(&msgResp);
            pPduResp->SetSeqNum(pPdu->GetSeqNum());
            pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_MSG);
            pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_MSG_UNREAD_CNT_RESPONSE);
    ...
    void setDevicesToken(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            pPduResp->SetPBMsg(&msgResp);
            pPduResp->SetSeqNum(pPdu->GetSeqNum());
            pPduResp->SetServiceId(IM::BaseDefine::DFFX_SID_LOGIN);
            pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_LOGIN_RES_DEVICETOKEN);
            CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        else
        {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (pCacheConn)
            {
                ...
                for (uint32_t i=0; i<nCnt; ++i) {
                    string strKey = "device_token:userId_"+int2string(msg.user_id(i));
                ...
                if(bRet)
                {
                    for (auto it=mapTokens.begin(); it!=mapTokens.end(); ++it) {
                        ...
                        size_t nPos = strKey.find("device_token:userId_");
                        if( nPos != string::npos)
                        {
                            string strUserId = strKey.substr(nPos + strlen("device_token:userId_"));
            ...
            pPduResp->SetCommandId(IM::BaseDefine::DFFX_CID_OTHER_GET_DEVICE_TOKEN_RSP);
            CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        else
        {
