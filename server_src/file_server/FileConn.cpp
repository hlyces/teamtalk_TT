...
void* _DoUpload(void* lparam)
{
	...
	for (uint32_t cnt = 0; cnt < 1; ++cnt)
	{
		...
		if (t->upload_packages.end() != iter)
		{
			...
			pdu.SetCommandId(DFFX_CID_FILE_PULL_DATA_REQ);
	...
	if(rename(t->save_path_fxtmp.c_str(), t->save_path.c_str()))
		log("!rename error %s --> %s", t->save_path_fxtmp.c_str(), t->save_path.c_str());
...
void CFileConn::Close()
{
	...
	pthread_rwlock_wrlock(&g_file_task_map_lock);
	TaskMap_t::iterator iter = g_file_task_map.find(m_strTaskId.c_str());
	if (g_file_task_map.end() != iter)
	{		
		transfer_task_t* t = iter->second;
		if(t)
...
void CFileConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId())
	{
		...
		case DFFX_CID_FILE_PULL_DATA_RSP:
			...
			break ;
		case DFFX_CID_OTHER_FILE_TRANSFER_REQ:
			_HandleMsgFileTransferReq(pPdu);
			break ;
		case DFFX_CID_OTHER_FILE_SERVER_IP_REQ:
...
void CFileConn::_HandleClientFileLoginReq(CImPdu* pPdu)
{
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_FILE);
	pdu.SetCommandId(DFFX_CID_FILE_LOGIN_RES);
	...
	CImPdu pdu2;
	pdu2.SetPBMsg(&msg2);
	pdu2.SetServiceId(DFFX_SID_FILE);
	pdu2.SetCommandId(DFFX_CID_FILE_LOGIN_RES);
	      ...
	      t->ready_to_recv))
	{
		...
		if(t->to_conn 
			&& FILE_TYPE_ONLINE==t->transfer_mode)
		{
			_StatesNotify(IM::BaseDefine::CLIENT_FILE_PEER_READY, task_id.c_str(), t->from_user_id, t->to_conn);//t->GetConn(m_user_id)
...
void CFileConn::_HandleMsgFileTransferReq(CImPdu* pPdu)
{
	...
	pdu.SetCommandId(DFFX_CID_OTHER_FILE_TRANSFER_RSP);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	char task_id[64] = {0};
...
void CFileConn::_HandleClientFilePullFileReq(CImPdu *pPdu)
{
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(DFFX_SID_FILE);
	pdu.SetCommandId(DFFX_CID_FILE_PULL_DATA_RSP);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	...
	switch (mode)
	{
		...
		case IM::BaseDefine::FILE_TYPE_OFFLINE: // for the offline download
		{
			...
			pdu2.SetPBMsg(&msg2);
			pdu2.SetServiceId(DFFX_SID_FILE);
			pdu2.SetCommandId(DFFX_CID_FILE_PULL_DATA_RSP);
			pdu2.SetSeqNum(pPdu->GetSeqNum());
			SendPdu(&pdu2);
			delete[] tmpbuf;
...
void CFileConn::_HandleClientFilePullFileRsp(CImPdu *pPdu)
{
	...
	string task_id = msg.task_id();
	uint32_t offset = msg.offset();
	uint32_t data_size = msg.file_data().length();
		...
		case FILE_TYPE_OFFLINE: /// this is the response to the server pull-data-req
		{
			...
			if (t->upload_packages.end() != itPacks)   // offset right
			{
				...
				if(USE_OLD_MEMFILE)
				{
					...
					memcpy(itPacks->second->data, msg.file_data().c_str(), msg.file_data().length());
				}
				else
				{
					//
					...
					{
						// open at first time
						t->fp = fopen(t->save_path_fxtmp.c_str(), "rb+");  // save fp
						if (!t->fp)
						{
							log("can not open file");
							...
							msg2.set_trans_mode((::IM::BaseDefine::FileType)t->transfer_mode);
							msg2.set_offset(offset);
							msg2.set_data_size(itPacks->second->size);
							CImPdu pdu;
							pdu.SetPBMsg(&msg2);
							pdu.SetServiceId(DFFX_SID_FILE);
							pdu.SetCommandId(DFFX_CID_FILE_PULL_DATA_REQ);
					...
					// offset file_header_t
					int iret = fseek(t->fp, sizeof(file_header_t) + offset, SEEK_SET); // write after file_header_t
					if (0 != iret)
					{
						log("seek offset failed.");
					...
					fwrite(msg.file_data().c_str(), 1, msg.file_data().length(), t->fp);

					/*---------------------------------*/
				}
				itPacks->second->isRecved = true;
...
int CFileConn::_StatesNotify(int state, const char* task_id, uint32_t user_id, CImConn* conn)
{
	if (!_IsAuth())
	{
		log("_StatesNotify !_IsAuth() !!! user %d state %d task %s", user_id, state, task_id);		
		return 1;
	}
	if(NULL==conn)
	{
		log("_StatesNotify conn is NULL !!! user %d state %d task %s", user_id, state, task_id);
		return 2;
	}
	...
	pdu.SetPBMsg(&msg);
	pdu.SetServiceId(DFFX_SID_FILE);
	pdu.SetCommandId(DFFX_CID_FILE_STATE);
...
int CFileConn::_PreUpload(const char* task_id)
{
	...
	t->save_path_fxtmp = t->save_path + STR_FXTMP_EXT;

	t->fp = fopen(t->save_path_fxtmp.c_str(), "wb+"); //"ab+"
	if (!t->fp)
	{
	...
	if(!USE_OLD_MEMFILE)
	{
		...
		//
		//
		seg_data = new char[SEGMENT_SIZE];
	}
	...
	for (uint32_t cnt = 0; cnt < total_packages; ++cnt)
	{
		...
		if(!USE_OLD_MEMFILE)
		{
			...
			fwrite(seg_data, 1, package->size, t->fp);
		}
	}
...
void CFileConn::_HandleGetServerAddressReq(CImPdu* pPdu)
{
	...
	pdu.SetPBMsg(&msg);
	pdu.SetServiceId(DFFX_SID_OTHER);
	pdu.SetCommandId(DFFX_CID_OTHER_FILE_SERVER_IP_RSP);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	SendPdu(&pdu);
	return;
}
