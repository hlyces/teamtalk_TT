...
void CRouteConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        case DFFX_CID_OTHER_HEARTBEAT:
            ...
            break;
        case DFFX_CID_OTHER_ONLINE_USER_INFO:
            _HandleOnlineUserInfo( pPdu );
            break;
        case DFFX_CID_OTHER_USER_STATUS_UPDATE:
            _HandleUserStatusUpdate( pPdu );
            break;
        case DFFX_CID_OTHER_ROLE_SET:
            _HandleRoleSet( pPdu );
            break;
...
void CRouteConn::_HandleOnlineUserInfo(CImPdu* pPdu)
{
	...
	for (uint32_t i = 0; i < user_count; i++) {
        IM::BaseDefine::UserStat server_user_stat = msg.user_stat_list(i);
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
{
    ...
    {//只是发给自己的不同客户端
        ...
        pdu.SetPBMsg(&msg2);
        pdu.SetServiceId(DFFX_SID_OTHER);
        pdu.SetCommandId(DFFX_CID_OTHER_LOGIN_STATUS_NOTIFY);
    ...
    //if (CHECK_CLIENT_TYPE_PC(client_type))  //注释掉是为了移动端上下线也应通知好友
    {
		...
		user_stat->set_client_type((IM::BaseDefine::ClientType)client_type);
        CImPdu pdu2;
        pdu2.SetPBMsg(&msg3);
        pdu2.SetServiceId(DFFX_SID_BUDDY_LIST);
        pdu2.SetCommandId(DFFX_CID_BUDDY_LIST_STATUS_NOTIFY);
        ...
        if (pUser)
        {
			...
			*/
			//上线都通知好友
			if(USER_STATUS_ONLINE == user_status)
            {
                _BroadcastMsg(&pdu2);
            }
        }
...
void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu)
{
    ...
    for(uint32_t i = 0; i < query_count; i++)
	{
        ...
        if (pUser) {
            ...
            user_stat->set_status((::IM::BaseDefine::UserStatType) pUser->GetStatus(client_type)) ;
			user_stat->set_client_type( (::IM::BaseDefine::ClientType)client_type);
        }
		else
		{
    ...
    pdu.SetCommandId(DFFX_CID_BUDDY_LIST_USERS_STATUS_RESPONSE);
