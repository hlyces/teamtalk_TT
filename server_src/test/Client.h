...
public:
    ...
    uint32_t getRecentSession(int time);
	uint32_t delRecentSession(uint32_t nSessionId, IM::BaseDefine::SessionType session_type);
	...
	uint32_t addFriendReq(uint32_t nToId,uint32_t nGroupId, const string& strRemark);
	uint32_t delFriendReq(uint32_t nToId);
	uint32_t chgFriendRemarkReq(uint32_t nToId, const string& strRemark);
	uint32_t getAddFriendReq();
	uint32_t findFriendReq(string find_string);

	uint32_t createFriendGroup(string group_name);
	uint32_t delFriendGroup(uint32_t  group_id);
	uint32_t moveFriendToGroup(uint32_t  group_id, list<uint32_t > friend_id_list);
	uint32_t chgFriendGroupName(uint32_t  group_id, string group_name);

	uint32_t fileReq( uint32_t to_user_id, string file_name, uint32_t type);
	uint32_t sendFileRequest(uint32_t to_user_id, string file_name, uint32_t file_size);

	uint32_t addOffFile( uint32_t to_user_id, string task_id, string file_name);
	uint32_t delOffFile( uint32_t to_user_id, string task_id);
	uint32_t hasOffFile();
