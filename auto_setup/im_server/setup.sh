...
build_im_server() {
	...
	if [ $? -eq 0 ]; then
		...
		cp -rf ./cert_file  $INSTALL_DIR/$IM_SERVER/$PUSH_SERVER/
		cp -f ./conf/$DB_PROXY_SERVER_CONF $INSTALL_DIR/$IM_SERVER/$DB_PROXY_SERVER/
		cp -f ./conf/$PUSH_CLIENT_CONF	$INSTALL_DIR/$IM_SERVER/$PUSH_CLIENT/
		...
		#cp -f ./$IM_SERVER/lib/* $INSTALL_DIR/$IM_SERVER/$PUSH_CLIENT/
